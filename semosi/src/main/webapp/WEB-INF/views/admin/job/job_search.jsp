...
<div class="page-wrapper">
	...
	<div class="body-container">
		...
		<div class="page-container">
			<form action="/job_search_list.sms" class="form-inline" id="formSearch">
					...
					<input type="button" value="선택삭제" class="btn btn-outline-info" onclick="deleteValue();">
						<table class="board">
							<tr class="head">
								...
								<th class='member_name'>작성자</th>
								<th class='title'>제목</th>
								<th class='date'>작성일</th>
								<th class='del_YN'>삭제</th>
							</tr>
							<c:choose>
								<c:when test="${fn:length(jobSearchList)!=0 }">
									<c:forEach var="jobSearchList" items="${jobSearchList}" varStatus="status">
										<tr class="contents">
											<td class='select'><input type="checkbox" value="${jobSearchList.postNo}" class="check" name="send-select" /></td>
											<td class='post_no'>${jobSearchList.postNo}</td>
											<td class='member_id'>${jobSearchList.memberId}</td>
											<td class='member_name'>${jobSearchList.memberName}</td>
											<td class='title'>
												<form action="/job_search_page.sms" method="post" class="title_form">
													<input type="hidden" value="${jobSearchList.postNo}" name="postNo"/>
													<button type="submit" name="title" class="title_btn" value="${jobSearchList.title}">${jobSearchList.title}</button>
												</form>
											</td>
											<td class='date'>${jobSearchList.writeDate}</td>
											<td class='del_YN'><c:choose>
											<c:when test="${jobSearchList.delYN eq 'N'.charAt(0)}">
												<button type="button" class="delete-btn delChangeSearch" id="${jobSearchList.postNo}" name="${jobSearchList.delYN}">삭제</button>
											</c:when>
											<c:otherwise>
												<button type="button" class="restore-btn delChangeSearch" id="${jobSearchList.postNo}" name="${jobSearchList.delYN}">복구</button>
											</c:otherwise>
										</c:choose>									
										</td>
										</tr>
							...
							<c:forEach begin="${fn:length(jobSearchList)}" end="12">
									<tr class="contents">
											<td class='select'></td>
											<td class='post_no'></td>
