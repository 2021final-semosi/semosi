	...
	<div class="page-wrapper">
		...
		<div class="body-container">
			...
			<div class="page-container">
				<!-- 페이징 처리를 위한 form -->
				<form class="form-inline" id="frmSearch" action="/offer_review.sms">
										...
										<c:when test="${fn:length(OfferReviewList)!=0 }">
											<c:forEach var="orList" items="${OfferReviewList}" varStatus="status">
												<tr class="contents">
													...
													<td class="grade">${orList.grade }</td>
													<td class='date'>${orList.writeDate}</td>
													<td class='delete-YN'>
													<c:choose>
														<c:when test="${orList.delYn eq 'Y'.charAt(0) }">
															<button type="button" class="delChangeBtn restore-btn" id="${orList.reviewNo }" name="${orList.delYn}">복구</button>
														</c:when>
														<c:otherwise>
															<button type="button" class="delChangeBtn delete-btn" id="${orList.reviewNo }" name="${orList.delYn}">삭제</button>
														</c:otherwise>
													</c:choose>
													</td>
												</tr>
